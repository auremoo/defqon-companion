...
public/          # Static assets, manifest, service worker
  mainstages/    # Optimized mainstage photos (generated by `npm run photos`)
photos-src/      # Photo originals, gitignored — source for `npm run photos`
...
npm run photos   # Optimize mainstage photos + regenerate their manifest
```
...
- **Mainstage photos are build-time assets** — originals live in `photos-src/` (gitignored), `npm run photos` emits WebP + thumbnails into `public/mainstages/` and a manifest the Photos tab fetches at runtime.
